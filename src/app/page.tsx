  ...
  return (
    <>
      ...
      <div ref={div0Ref} className={`w-screen flex flex-col items-center justify-center ${mobileView ? "h-[50vh]" : "h-[100vh]"}`}>
        <div ref={div0Ref} className={`relative w-screen overflow-hidden ${mobileView ?" h-[40vh]":"h-7/8"}`}>
        ...
        <div className={`h-[150px] w-screen p-4 ${mobileView ? "text-center" : ""} flex flex-col items-center justify-center gap-2 ${mont.className} backdrop-blur-md transition-colors duration-500 text-black`}>
          <h1 className={`tracking-wide ${mobileView ? "text-base" : "text-xl"} font-bold text-earth-brown`}>Velavadar National Park and itsWildlife</h1>
          <h2 className={`${mobileView ? "text-xs" : "text-lg"}  text-earth-brown`}>What makes Blackbuck National Park, Velavadar the best places to see Indian Wolves, Blackbucks, Montagu Harrier and Lesser Florican</h2>
        </div>
      </div>
      ...
      (
        <>
          <div ref={div1Ref} className={`h-[400px] w-screen relative z-10 gap-4 flex items-center justify-center text-black ${mont.className}`}>
            {/* From Left */}
            <motion.div
              className={`h-full w-1/2 px-3 flex flex-col items-center justify-center text-earth-brown ${mobileView ? "pl-2 gap-3" : "pl-10 gap-7"}`}
              initial={{ opacity: 0 }}
              animate={(div1InView || div2InView) ? { opacity: 1 } : {}}
              transition={{ duration: 1.2, ease: "easeOut" }}
            >
              <div className="w-full flex flex-col items-end justify-between gap-2">
                <span className={`${mobileView ? "text-xs" : "text-base"} text-right`}>Blackbuck National Park is a hidden realm holding all the gems of nature under one roof. The beautiful views of the exotic and exquisite wildlife and grasslands compel travelers from all over the world to visit and spend time in nature’s bounty. Lying between two rivers, Parvaila and Alang, which flow into the Gulf of Khambhat, you can witness many wolves living and strolling around these water bodies.</span>
              </div>
              <div className="w-full flex flex-col items-end gap-2">
                <span className={`${mobileView ? "text-xs" : "text-lg"} text-right font-bold `}>Want to see a grassland ecosystem with Indian Wolves, herd of blackbuck in India?</span>
                <span className={`${mobileView ? "text-sm" : "text-base"} text-right `}>Indian Wolf sightings are difficult, but if you do sight one, it’s an unparalleled experience. Though at Velavadar, blackbucks are sighted easily. If you are lucky, a blackbuck territorial fight is a rare sight you might witness.</span>
              </div>
            ...
            >
              <Image src={images[2].src} alt={images[2].altText} fill sizes="100vw" className="object-contain transition-all duration-300"/>
            </motion.div>
        ...
        <>
          <div ref={div1Ref} className={`h-[50vh] w-full relative flex items-center justify-center px-4 py-6 text-center ${mont.className}`}>
            <Image
              src={images[2].src} alt={images[2].altText} fill sizes="100vw" className="object-cover z-0" priority/>
      ...
      <motion.div 
        ref={div2Ref} 
        className={`${mobileView?"h-auto":"h-[1000px]"} w-screen relative z-10 flex flex-col items-center justify-center text-3xl text-black pt-10 bg-transparent`}
      ...
      <div className={` w-full flex ${mobileView?"flex-col h-[500px]":"h-1/2"} items-center justify-center gap-7 home-page-bg-div2 ${mont.className}`}>
          ...
          <motion.div className="absolute inset-0 flex items-center justify-center">
            <motion.div className={`relative z-10 text-white px-10 w-full h-2/3 flex flex-col justify-between items-start text-left`}>
              <motion.div 
                ...
                initial={mobileView ? { opacity: 1, y: 0 } : { opacity: 0, y: 50 }}
                animate={mobileView ? { opacity: 1, y: 0 } : (div2InView || div3InView ? { opacity: 1, y: 0 } : { opacity: 0, y: 50 })}
                transition={{ duration: 0.8, ease: "easeOut" }}
              >
                <h2 className={`${mobileView ? "text-xs" : "text-lg"} font-normal`}>
                  A Luxury Hotel in Velavadar Park
                </h2>
                <h1 className={`${mobileView ? "text-sm" : "text-xl"} font-bold`}>
                  The Blackbuck Lodge
                </h1>
              </motion.div>
        ...
        <div className={`${mobileView?"h-[700px] mt-4":"h-1/2"} w-full flex items-center justify-center`}>
          <div className={`h-full w-4/5 flex ${mobileView?"flex-col":""} items-center justify-center`}>
            <div className={`h-full ${mobileView?"w-full":"w-3/5 pr-10"} flex flex-col items-start justify-center text-left gap-3 text-black ${mont.className}`}>
              ...
              <h2 className={`${mobileView?"text-xs":"text-base"} font-normal text-earth-brown`}>Being a safe space for many unique mammals and a dedicated conservatory of blackbucks, the Blackbuck national park is a beautiful place to witness the beauty of nature and wildlife in its most ideal form. Its unique grassland ecosystem is one of the most fascinating parts of this park.</h2>
              <motion.button
            ...
            <div className={`h-full ${mobileView?"w-full":"w-2/5"} flex flex-col items-center justify-center text-black text-center ${mont.className}`}>
              <h2 className={`${mobileView?"text-xs":"text-base"} font-semibold text-earth-brown`}>Book your stay and safari in Blackbuck National Park</h2>
              <h1 className={`${mobileView?"text-sm":"text-xl"} font-bold text-earth-brown`}>Stay at Velavadar Blackbuck National Park</h1>
              <motion.button
                className={`relative border-2 border-earth-brown px-3 py-2 my-3 rounded-full ${mobileView?"text-xs":"text-sm"} font-light cursor-pointer`}
                initial={{ backgroundColor: "rgba(0, 0, 0, 0)", backdropFilter: "blur(0px)" }}
                whileHover={{
                  backgroundColor: "rgba(255, 255, 255, 1)",
                  backdropFilter: "blur(7px)",
      ...
      {/* div3 */}
      <div ref={div3Ref} className={`${mobileView ? "h-[1750px]" : "h-[950px]"} w-screen relative z-10 flex items-center justify-center text-3xl text-black bg-earth-brown ${mont.className}`}>
        <div className={`${mobileView ? "h-[1650px]" : "h-[800px]"} w-4/5 bg-black/54 rounded-xl flex ${mobileView ? "flex-col" : ""} items-center justify-center`} style={{ backdropFilter: 'blur(7px)' }}>
          ...
          <div className={`h-full ${mobileView ? "w-full" : "w-2/3"} flex flex-col items-start justify-center p-7 gap-3`}>
            ...
            <div className="w-full h-[400px] overflow-x-auto scrollbar-hide">
              <motion.div
                ref={ref}
                className="flex gap-4 h-full w-full py-4 cursor-all-scroll"
                style={{
              ...
              >
                {galleryImages.map((image) => (
                  <div
                    key={image.id}
                    className={`relative h-full ${mobileView?'w-full':'w-1/3'} flex-shrink-0 snap-start`}
                  >
                    <Image
                      src={image.src}
                      alt={image.alt}
            ...
            <div className='h-auto w-full flex items-center justify-start gap-4'>
              <motion.button
                className={`relative border-2 border-white px-3 py-2 my-3 rounded-full ${mobileView?"text-xs":"text-sm"} font-light bg-transparent cursor-pointer ${mont.className}`}
                initial={{
      ...
      {/* div4 */}
      <div ref={div4Ref} className={`relative ${mobileView?"h-[400px]":"h-[1000px]"} w-screen overflow-x-hidden flex flex-col items-center justify-center`}>
      {/* Header Section */}
      <div className={`relative ${mobileView?"h-[100px]":"h-[200px]"} w-full bg-transparent flex flex-col text-center items-center justify-center gap-3 p-7 text-black ${mont.className} z-10`}>
      ...
      {mobileView ? (
        <div className="relative w-full h-[300px] px-4 overflow-visible">
          <div className="h-[30vh] w-full overflow-hidden">
            <motion.div
              ref={ref}
              className="flex gap-4 h-full w-full overflow-x-auto scrollbar-hide py-4 cursor-all-scroll"
              style={{
                scrollSnapType: 'x mandatory',
                scrollBehavior: 'smooth'
              }}
            >
              {div4Images.map((image) => (
                <div
                  key={image.id}
                  className="relative h-full w-2/3 flex-shrink-0 snap-start"
                >
                  <Image
                    src={image.src}
                    alt={image.alt}
      ...
      <motion.div 
        ref={div5Ref} 
        className={`${mobileView?"h-auto mt-4":"h-[1000px]"} w-screen bg-white relative z-10 overflow-hidden flex flex-col items-center justify-center gap-6 text-black`}
        initial={mobileView?{ opacity: 1}:{ opacity: 0, y: 50 }}
        animate={mobileView?{}:(div5InView||div6InView) ? { opacity: 1, y: 0 } : { opacity: 0, y: 50 }}
        transition={{ duration: 0.8, ease: "easeOut" }}
      >
        {/* Title */}
        <div className='h-[150px] w-2/3 flex items-center justify-center py-4 mt-5'>
          <span className={`${mobileView?"text-base text-center":"text-2xl"} font-bold text-earth-brown ${mont.className}`}>
            About Hotels and Resorts in Blackbuck National Park,Velavadar
          </span>
        </div>
        ...
        <div className={` w-full px-10 ${mobileView ? 'flex flex-col gap-4 h-[1000px]' : 'grid grid-cols-1 md:grid-cols-2 gap-4 h-[800px]'} ${mont.className}`}>
          {[
            ...
            >
              {/* Image */}
              <Image src={item.src} alt={item.alt} layout="fill" className="object-cover" />
      ...
      {/* Home Page Div 6 */}
      <div ref={div6Ref} className={`${mobileView?"h-[800px]":"h-[600px]"} w-screen flex flex-col items-center justify-center relative`}>
        <div className={`h-auto w-full flex ${mobileView?"flex-col":""} items-center justify-center gap-2`}>
          <div className={`h-full ${mobileView?"w-2/3 text-center":"w-1/3"} flex flex-col items-start justify-start gap-3 p-4`}>
            <span className={`text-earth-brown ${mobileView?"text-sm":"text-2xl"} font-extrabold ${mont.className}`}>Conservation Efforts at Blackbuck National Park</span>
            <span className={`text-earth-brown ${mobileView?"text-xs":"text-xl"} font-extrabold ${mont.className}`}>Responsible Tourism in Velavadar</span>
            <span className={`text-earth-brown ${mobileView?"text-xs":"text-base"} font-normal ${mont.className}`}>The conservation of an endangered species of blackbucks is the main highlight of this park. Once, blackbucks could be seen roaming around the entire subcontinent, but today its population is concentrated within the borders of Gujarat, India. The steady focus and thoughtful conservation in protecting these endangered species have ultimately proved to be a fruitful endeavour. From 200 in the 1960s to above a whopping 3000 today, the efforts of preserving these almost extinct animal species have paid off.</span>
          </div>
          <div className={`h-full ${mobileView?"w-2/3 text-center":"w-1/3"} flex flex-col items-start justify-start gap-3 p-4`}>
            <span className={`text-earth-brown ${mobileView?"text-sm":"text-2xl"} font-extrabold ${mont.className}`}>Why visit Blackbuck National Park at Velavadar?</span>
            <span className={`text-earth-brown ${mobileView?"text-xs":"text-xl"} font-extrabold ${mont.className}`}>Wildlife at Blackbuck National Park</span>
            <span className={`text-earth-brown ${mobileView?"text-xs":"text-base"} font-normal ${mont.className}`}>The national park is also home to a large number of exotic and unique biodiversity. The sanctuary is one of the most suitable places to see striped hyenas, a rarely seen species. You can also witness the prime predator of the park, the Indian wolf. A walk through the park can give visitors a chance to catch sights of many smaller but beautiful mammals such as jungle cats, hare, golden jackal, and Indian foxes.</span>
          </div>
        </div>
        <motion.button
          className={`relative border-2 border-earth-brown px-3 py-2 my-3 rounded-full ${mobileView?"text-xs":"text-sm"} font-light cursor-pointer ${mont.className}`}
      ...
      {/* Home Page Div 7 */}
      <div ref={div7Ref} className={`${mobileView?"h-[1250px]":"h-[1000px]"} bg-white w-screen flex items-center justify-center relative`}><WhyUs/></div>
      {mobileView ? (
        <div
          ref={div8Ref}
          className="min-h-[900px] mt-10 w-screen bg-earth-brown flex flex-col items-center justify-center relative"
        >
          {/* Section Heading */}
          <div
            className={`text-white text-base mb-4 h-[100px] w-full flex items-center justify-center my-7 font-extrabold ${mont.className}`}
          >
            How to reach Blackbuck National Park?
          </div>

          {/* Ahmedabad Block */}
          <div className="w-full px-4 flex flex-col items-center justify-center mb-10">
            <div
              className={`w-full text-center text-white text-sm mb-4 font-normal ${mont.className}`}
            >
              Reaching from Ahmedabad Airport
            </div>
            <div className="w-4/5 border-2 bg-white/77 border-earth-brown">
              <iframe src="https://www.google.com/maps/embed?pb=!1m28!1m12!1m3!1d943215.1111480193!2d71.78578221849111!3d22.562864641009348!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!4m13!3e0!4m5!1s0x395e814e9704ec85%3A0x90dd00e5a99ba47a!2sSardar%20Vallabhbhai%20Patel%20International%20Airport%2C%20Hansol%2C%20Ahmedabad%2C%20Gujarat%20380003!3m2!1d23.0734262!2d72.626571!4m5!1s0x395f357bf4279847%3A0x845851d20b1bb837!2s22VC%2BJ4R%20Blackbuck%20National%20Park%2C%20Adhelai-Velavadar%20Link%20Road%2C%20Gujarat%20364313!3m2!1d22.044107999999998!2d72.020281!5e0!3m2!1sen!2sin!4v1651394721603!5m2!1sen!2sin" 
              width="100%"
              height="400px"
          ...
          <div className="w-full px-4 flex flex-col items-center justify-center mb-10">
            <div
              className={`w-full text-center text-white text-sm mb-4 font-normal ${mont.className}`}
            >
              Reaching from Bhavnagar Airport
            </div>
            <div className="w-4/5 border-2 bg-white/77 border-earth-brown">
              <iframe src="https://www.google.com/maps/embed?pb=!1m28!1m12!1m3!1d236912.6960502062!2d71.91455909768017!3d21.905282086323492!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!4m13!3e0!4m5!1s0x395f5a4a7dc4bd1b%3A0x2a35950c2387995f!2sBhavnagar%20Airport%2C%20Bhavnagar%2C%20Gujarat!3m2!1d21.7537804!2d72.1834267!4m5!1s0x395f357bf4279847%3A0x845851d20b1bb837!2s22VC%2BJ4R%20Blackbuck%20National%20Park%2C%20Adhelai-Velavadar%20Link%20Road%2C%20Gujarat%20364313!3m2!1d22.044107999999998!2d72.020281!5e0!3m2!1sen!2sin!4v1651394771831!5m2!1sen!2sin" 
              width="100%"
              height="400px"
              style={{ width: '100%', height: '400px' }}
              allowFullScreen
        ...
        <div
          ref={div8Ref}
          className="h-[700px] w-screen bg-earth-brown flex flex-col items-center justify-center relative"
        >
          <div className={`text-white text-2xl h-[100px] w-full flex items-center justify-center my-7 font-extrabold ${mont.className}`}>
            How to reach Blackbuck National Park?
          </div>

          <div className="w-full flex h-[600px] items-center justify-center gap-10">
            {/* Ahmedabad */}
            <div className="h-[400px] w-2/3 flex flex-col items-center justify-center">
              <span className={`text-white text-xl mb-7 font-normal ${mont.className}`}>
                Reaching from Ahmedabad Airport
              </span>
              <div className="h-full w-2/3 border-2 bg-white/77 border-earth-brown">
                <iframe src="https://www.google.com/maps/embed?pb=!1m28!1m12!1m3!1d943215.1111480193!2d71.78578221849111!3d22.562864641009348!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!4m13!3e0!4m5!1s0x395e814e9704ec85%3A0x90dd00e5a99ba47a!2sSardar%20Vallabhbhai%20Patel%20International%20Airport%2C%20Hansol%2C%20Ahmedabad%2C%20Gujarat%20380003!3m2!1d23.0734262!2d72.626571!4m5!1s0x395f357bf4279847%3A0x845851d20b1bb837!2s22VC%2BJ4R%20Blackbuck%20National%20Park%2C%20Adhelai-Velavadar%20Link%20Road%2C%20Gujarat%20364313!3m2!1d22.044107999999998!2d72.020281!5e0!3m2!1sen!2sin!4v1651394721603!5m2!1sen!2sin" 
              width="100%"
              height="400px"
              style={{ width: '100%', height: '400px' }}
      ...
      {/* Home Page Div 8 */}
      <div ref={div9Ref} className='min-h-[800px] w-screen flex items-center justify-center relative'>
          ...
          <div className="h-3/4 w-full overflow-y-auto">
            {faqs.map((faq, index) => (
              <div key={index} className="border-b-2 border-earth-brown p-4">
                ...
                <div 
                  className={`overflow-hidden ${mont.className} text-earth-brown ${mobileView ? "text-xs" : "text-base"} font-normal transition-all duration-500 ease-in-out ${
