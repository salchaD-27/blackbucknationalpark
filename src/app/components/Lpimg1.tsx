...
export default function Lpimg1(){
  ...
  useEffect(() => {
    ...
    return(
        <div className={`h-full w-full flex flex-col items-start justify-end ${mobileView ? "" : "gap-4"}`}>
            <span className={`${mobileView ? "text-xs" : "text-2xl"} font-normal ${mont.className}`}>Rustic Accommodation</span>
            <span className={`${mobileView ? "text-sm" : "text-6xl"} font-bold ${mont.className}`}>Luxury Cottages with</span>
            <span className={`${mobileView ? "text-sm" : "text-6xl"} font-bold ${mont.className}`}>all Modern Amenities</span>
            <motion.button
