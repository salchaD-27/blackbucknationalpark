...
const AnimatedMenuButton = ({ div0View, setIsOpen, isOpen }: MenuButtonProps) => {
  return (
    <button
      ...
      className={`z-50 p-3 ${div0View?"bg-white/81":"bg-black/81"}  shadow-lg rounded-full flex items-center justify-center`}
    >
      <motion.div
        initial="closed"
        ...
        className="relative w-6 h-6 flex flex-col justify-center gap-1"
      >
        {/* Top Line */}
        <motion.div
          className={`w-6 h-0.5 ${div0View?"bg-black":"bg-white"} rounded-full`}
          variants={{
            closed: { rotate: 0, y: 0 },
            open: { rotate: 45, y: 8 },
          }}
...
export default function Header({div0View}:HeaderProps){
    ...
    return(
      <>
  {/* Header Bar */}
  <div className={`${mobileView? (div0View ? "h-[5vh] bg-white/30" : "h-[3vh] bg-black/30" ): (div0View ? "h-[9vh] bg-white/30" : "h-[7vh] bg-black/30")} w-2/3 transition-all duration-300 fixed top-4 left-1/2 -translate-x-1/2 z-[100] backdrop-blur-md flex items-center justify-center rounded-full shadow-lg`}>
    <div className="h-full w-4/5 flex items-center justify-center">
      
      {/* Logo Button */}
      <div className="h-full w-1/5 flex items-center justify-start px-4">
        <motion.button
          className="w-1/2 h-4/5 bg-contain bg-center text-black text-3xl font-extrabold cursor-pointer relative overflow-hidden"
  ...
  {isOpen && (
    <motion.div
      ...
      className="fixed top-0 right-0 h-screen w-1/4 header-menu-bg-gradient backdrop-blur-xl shadow-xl flex flex-col items-start justify-center gap-4 px-6 py-10 z-[90]"
    >
      {menuItems.map((item, index) => (
        <div key={index} className="flex flex-col items-start justify-center">
          ...
          {/* Main Menu Item */}
          <motion.button
            className={`text-lg font-bold text-white/90 py-1 px-4 cursor-pointer ${mont.className}`}
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
          ...
          {item.subItems.length > 0 && (
            <div className="flex flex-col items-start justify-center gap-1">
              {item.subItems.map((subItem, subIndex) => (
                <motion.button
                  key={subIndex}
                  className="text-base font-bold text-white/70 cursor-pointer px-4"
